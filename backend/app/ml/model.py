...
3. Sinon → entraîner un modèle HealthCare de démo à la volée (mode démo)
"""
...
from app.ml.healthcare_model import EXPECTED_FEATURE_COUNT, train_healthcare_model
...
def _train_healthcare_fallback():
    """Entraîne un classifieur HealthCare de démo et le sauvegarde."""
    logger.warning("Aucun modèle HealthCare compatible disponible, entraînement du fallback")
    clf = train_healthcare_model()
    MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
...
def load_model():
    ...
    if MODEL_PATH.exists():
        ...
        if MODEL_S3_BUCKET:
            raise ValueError(
                ...
                f"mais HealthCare en fournit {EXPECTED_FEATURE_COUNT}."
            )
        logger.warning(
    ...
    return _train_healthcare_fallback()
