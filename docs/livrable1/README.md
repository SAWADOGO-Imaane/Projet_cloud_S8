...
**Exemple pour HealthCare :**
- **Problème :** Fournir un premier diagnostic médical assisté par IA pour réduire le temps d'attente aux urgences
- **Utilisateurs :** Infirmiers, patients en auto-diagnostic
- **Modèle ML :** RandomForest sur 17 symptômes → 6 maladies, ~500 exemples d'entraînement, <1 ms d'inférence
- **Cloud :** AWS ECS Fargate pour l'API, RDS PostgreSQL, S3 pour le modèle
...
| Diagnostic médical faux              | Élevé  | Moyenne     | Disclaimer légal, validation données entrantes |
| Données patient sensibles exposées   | Élevé  | Faible      | Chiffrement BD, pas de logs sensibles, GDPR |
...
|----------------------|--------------------------|--------------|
| Compute (backend)    | ECS Fargate 0.25 vCPU    | 8 $          |
| Base de données      | RDS PostgreSQL 1 GB      | 10 $         |
| Stockage modèle ML   | S3 (50 MB modèle)        | 1 $          |
| ALB                  | Application Load Balancer| 12 $         |
| Logs CloudWatch      | 10 GB/mois               | 2 $          |
| **Total**            |                          | **~33 $**    |

**Hypothèses :** 
- 100 utilisateurs/mois
- ~300 diagnostics/mois = 10/jour
- Inférence ML <100ms par diagnostic
- Frontend servi par nginx sans compute (considéré dans S3 + CloudFront)

Exemple pour HealthCare avec moins de ressources :
- Compute : 4 $ (Fargate nano)
- PostgreSQL : 0 $ (AWS free tier si nouveau compte)
- S3 : 0,5 $
- Domaine : 0 $ (Let's Encrypt)
- **Total minimum : ~4,5 $/mois**
